...
A simple MQTT benchmarking tool for SupeMQ platform.

It connects SupeMQ clients as subscribers over a number of channels and
uses other SupeMQ clients to publish messages and create MQTT load.

SupeMQ clients used must be pre-provisioned first, and SupeMQ `provision` tool can be used for this purpose.
...
```
./mqtt-bench --help
Tool for extensive load and benchmarking of MQTT brokers used within SupeMQ platform.
...
Flags:
  ...
  -m, --supermq string   config file for SupeMQ connections (default "connections.toml")
...
Before use you need a `mgconn.toml` - a TOML file that describes SupeMQ connection data (channels, clientIDs, clientKeys, certs).
